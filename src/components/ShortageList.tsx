    ...
    <div className="bg-slate-800 rounded-xl border border-slate-700 shadow-md overflow-hidden flex flex-col">
      ...
      <div className="p-5 border-b border-slate-700 bg-slate-800/50 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="relative max-w-xs w-full">
            ...
            <input
              ...
              className="w-full text-xs pl-9 pr-4 py-2 border border-slate-700 rounded-lg focus:outline-none focus:ring-1 focus:ring-hanwha-orange bg-slate-900 text-slate-100 placeholder-slate-500"
            />
          </div>
        ...
        <button
          ...
          className="px-4 py-2 bg-slate-700 hover:bg-slate-600 border border-slate-600 disabled:opacity-50 text-slate-200 rounded-lg text-xs font-semibold flex items-center justify-center gap-1.5 transition-colors shadow-xs cursor-pointer"
        >
          {downloadSuccess ? (
            <>
      ...
      <div className="overflow-x-auto">
        <table className="w-full border-collapse text-left text-xs font-sans">
          <thead>
            <tr className="bg-slate-900/40 text-slate-400 font-medium border-b border-slate-700">
              <th className="py-3 px-4 font-semibold">자재명 / 자재코드</th>
              <th
                onClick={() => handleSort("status")}
                className="py-3 px-4 font-semibold cursor-pointer hover:bg-slate-700/50 transition-colors"
              >
              ...
              <th className="py-3 px-4 font-semibold">수급률 (입고 / 필요)</th>
              <th
              ...
              <th className="py-3 px-4 font-semibold">공정 단계</th>
              <th className="py-3 px-4 font-semibold">책임 협력사</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-700/50 text-slate-200">
            {paginatedMaterials.length === 0 ? (
              ...
              paginatedMaterials.map((item) => {
                ...
                return (
                  <tr key={item.materialCode} className="hover:bg-slate-700/20 transition-colors">
                    {/* Name & Code */}
                    <td className="py-3 px-4">
                      <div>
                        <p className="font-semibold text-slate-100 leading-normal">{item.materialName}</p>
                    ...
                    <td className="py-3 px-4 font-mono font-medium text-slate-100">
                      {item.shortageQty > 0 ? (
                        <span className="text-red-400 font-semibold">-{item.shortageQty} 개</span>
                      ) : (
                        <span className="text-emerald-400 font-semibold">충족 완료</span>
                      )}
                    </td>

                    {/* Target Date */}
                    <td className="py-3 px-4 font-mono text-slate-300 whitespace-nowrap">{item.targetDate}</td>

                    {/* Process Stage */}
                    <td className="py-3 px-4 text-slate-400 max-w-[140px] truncate" title={item.processStage}>
                      {item.processStage}
                    </td>

                    {/* Supplier */}
                    <td className="py-3 px-4 text-slate-400 font-medium truncate max-w-[140px]" title={item.supplier}>
                      {item.supplier}
                    </td>
                  </tr>
                );
      ...
      {/* Pagination Controls */}
      <div className="p-4 border-t border-slate-700 bg-slate-800/50 flex items-center justify-between text-xs text-slate-400">
        <p>
          총 <span className="font-semibold text-slate-200">{sortedMaterials.length}</span>개 중{" "}
        ...
        <div className="flex items-center gap-1.5">
          <button
            ...
            className="p-1 border border-slate-700 bg-slate-900/50 rounded-md hover:bg-slate-700 disabled:opacity-40 transition-colors cursor-pointer text-slate-300"
          >
            <ChevronLeft size={14} />
          </button>
